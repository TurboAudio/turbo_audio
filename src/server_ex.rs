...
pub fn main() {
	...
	let server = Server::bind("127.0.0.1:9001", &tokio::reactor::Handle::default())
	...
	let main_loop = future::loop_fn((), move |_| {
		...
		tokio::time::interval::new(Instant::now() + Duration::from_millis(1000))
			.map_err(|_| ())
			.and_then(move |_| {
...
fn update(
	...
	executor.spawn(futures::lazy(move || {
		for (id, _) in connections.read().unwrap().iter() {
			let f = channel.clone().send((*id, "Caca".to_owned()));
